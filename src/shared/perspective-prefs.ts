 ...
 *
 * Before H.22 §P2-1 these constants were copy-pasted into each view; keeping one
 ...
 * never-throw pattern FolderViz pioneered (plan §H.20 D): preferences are
...
/** Shown by default — workflow/priority/date/geo are noise in a frequency cloud
 ...
 * mode) errors — a failed save must never surface to the user.
 */
export function writePrefs<T>(key: string, value: T): void {
  try {
    localStorage.setItem(key, JSON.stringify(value));
  } catch {
    // localStorage full / disabled — prefs are convenience, not core data
  }
}
