 ...
 *
 * Builds a bipartite tag↔file graph from the current directory's tagged files
 ...
 * isolation (knowledge-graph.test.ts). No `Math.random` — layout is a pure
 * function of its input so the same graph always lays out identically.
 *
 * Previously named `mindmap.ts`; renamed in plan §H.19 to reflect that the
 * output is a bipartite tag↔file graph rather than a hierarchical mind map.
 */
...
/**
 * Build a bipartite tag↔file graph from per-file tags.
 *
 * - Files are looked up in `tagsByName` (by `entry.path` — H.24 R1 made the
 ...
 * across the canvas without clustering — see `seedPositions`.
 */
...
/** Rough rendered text width (px): CJK/full-width glyphs ≈ font size, latin ≈ 0.58×. */
 ...
 * `FileNode` components — if the estimate is too small, nodes will overlap
 ...
 *   2. **File chip ext span**: NOT a fixed 30px — it grows with the ext
 ...
 *   3. **CJK width coefficient**: real fonts render CJK at ~1.0× em,
 *      Latin at ~0.58×. The `textWidth` helper handles that — but we
...
export function nodeSize(node: MindMapNode): { w: number; h: number } {
  ...
  if (node.kind === 'tag') {
    ...
    // Actual style: padding `${4*scale}px ${10*scale}px` → horizontal 20*scale.
    // boxShadow: `0 1px 4px rgba(0,0,0,0.3)` extends ~4px below — already
    ...
    return {
      ...
      h: fontPx + 12 + 4, // line-height (~1.2× fontPx) + padding 4+4 + shadow 4
    };
  }
  ...
  // For long ext strings (DRAWIO, MARKDOWN, …) the chip grows; using the
 ...
 * same way. O(n²·iterations) — fine for a directory's worth of nodes.
 */
...
/**
 * Adaptive canvas size. The old `max(1000, √N × 260)` rule grew the canvas
 * geometrically with node count — at N=100 the canvas hit 2600px while the
 ...
 * (so `k = span × 0.04` lands around 32–96px depending on graph size), with
 ...
 * verbatim — only *new* nodes are seeded. This is how the view preserves
...
function seedPositions(
  ...
  // never collide (chord ≥ 88px vs required 84px). The old `tension`
  ...
  // arc shrinkage `2r·sin(θ/2) < rθ`). Without this, 30 tags at span=1300
  ...
  // made the `tension` slider invisible — raising requiredRadius
  ...
  files.forEach((fn, idx) => {
    ...
    dx /= len;
    dy /= len;
    // Halton 2D sequence — places each file at a unique, uniformly-spread
    ...
    // (with a small buffer) rather than a fraction of the canvas — the
    ...
    // ring size and 30 files in a 1500×1500 canvas get the full area.
    // Deterministic (no Math.random) — `idx + 1` because Halton(0) is
    // degenerate.
 ...
 * center. Skipped when `previousPos` is in play — that would defeat the whole
 * point of position memory. When the seed runs cold (no previousPos) and the
 * radial layout drifts off-center (common with `width ≠ height` or sparse
 * graph shapes), this restores the visual "node cluster in the middle of the
 * viewport" expectation. ~5 lines, fixes the "进场节点不在图像中间" complaint.
 */
 ...
 * any remaining overlaps. Returns a map of node id → {x, y}. Pure: same graph →
 * same positions.
 *
...
export function layoutGraph(
  ...
  //    No `tension` parameter anymore — the previous slider was removed
  ...
  //    adds chaos. In dense graphs the cumulative file↔file repulsion
  ...
  //    `physics:true, collide:true` puts f1 and f7 at x=30 — the left
  //    margin — overlapping in y). `physics` is kept as an option so
  ...
  // 3. Optional collision separation. 200 + 80 iterations at gap=20 —
  //    earlier 100 + 40 starved on dense many-to-many graphs where the
  ...
  //    if the cluster was strongly off-center — we re-clamp *after*
  //    recenter so the final positions are guaranteed inside the canvas.
  recenter(pos, canvas, hasMemory);

  // 6. Final clamp — guarantees every node is inside the canvas with a
...
/**
 * Weak file↔file repulsion pass. Mutates `pos` in place from its Halton
 * seed to push apart pairs of files that happen to land too close after
 * the 2D quasi-random seed. Skips tag↔tag pairs entirely — those would
...
function runForcePass(
  ...
  // Identify file and directory nodes for the repulsion below — tags get a
