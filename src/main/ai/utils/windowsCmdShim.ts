/**
 * Windows `.cmd` shim handling — verbatim port of Claudian's
 * `src/utils/windowsCmdShim.ts`. Pure logic, no Obsidian coupling.
 *
 * The Claude Code CLI is sometimes installed as a `.cmd` shim (npm-global on
 * Windows). Spawning a `.cmd` directly with `shell:false` fails, so we rewrap
 * it as `cmd.exe /d /s /c "<quoted command>"` with `windowsVerbatimArguments`,
 * and remember to kill the whole process tree on abort (the .cmd spawns a
 * child node process that `child.kill` alone would orphan).
 */
...
export function resolveWindowsCmdShimSpawnSpec(
  ...
  if (
    ...
    return { args: spec.args, command: spec.command };
  }
