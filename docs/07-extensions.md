...
| pdf-viewer | 扩展 iframe 内 pdfjs 浏览器版 + `HostBinaryDataFactory`(经 host IPC 拿 cmap / 标准字体 / wasm)+ `<canvas>` 渲染 + fit W/P + 旋转 + 状态栏;CJK 字体自动回退系统。**大文件 Range 流式**(2026-07-25):pdfjs **自定义 range transport**(`WhaleRangeTransport`)经 postMessage 桥到主进程 `fs:readFileRange` 按需取 64KB 分片(先取文件尾 xref,再按页取对象),**首页在几次小读取后即绘出,不再等整份读入回传**;整份字节桥(`requestFileBytes` 无 offset → `renderPdfBytes`)降级为缺 size / transport 失败时的回退。机制选择说明:URL Range 路径已证死路——Chromium 对 XHR 的 scheme 白名单硬编码(chrome/data/http/https),`supportFetchAPI` 只放行 fetch() 不放行 XHR,而 pdfjs legacy build 用 XHR;transport 路径不经过任何 URL fetch,天然规避。历史:2026-07-18 曾把 base64 整份改为 Uint8Array 结构化克隆(消 O(n²) 与 33% IPC 膨胀,但仍需整份读入);2026-07-23 曾试 whale-file CORS 回显(无效,已撤);可选真 worker(`USE_PDFJS_WORKER`,`pdf.worker.mjs` 经 `whale-extension://` 加载)把解析移出主线程。**框选提问**(2026-07-23):工具栏 ✨ toggle → 拖框命中 textLayer spans(client rect,旋转天然兼容)→ `marquee.ts` 按阅读顺序拼文本 → 迷你条「问 AI」→ `askAi` 消息 → host 开 AI 面板 + `whale:ai-draft` CustomEvent 投递附件草稿(见 docs/11 §16.3);**区域截图常带**(裁剪 page canvas ≤1400px PNG;扫描件无文本时走模型视觉,提示"将发送截图");`setAiAvailable` 不可用即隐藏按钮。office-viewer 仍走内存 bytes(LibreOffice 转换产物) |
