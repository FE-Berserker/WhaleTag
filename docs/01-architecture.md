...
`.erb/configs/webpack.config.{main,renderer}.{dev,prod}.ts` 多目标;原生模块靠 webpack externals + builder `asarUnpack`。
...
`electron --test --require ts-node/register`(非 jest),70+ 个 `*.test.ts(x)` 文件。

**新测试文件**必须手动加进 `package.json` 的 `test` 脚本(脚本读文件路径,不递归 glob)。
