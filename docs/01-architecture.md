...
- **左栏标签页**:[MainLayout](../src/renderer/containers/MainLayout.tsx) viewport ≤ 1400px 时,Sidebar(位置)+ DirectoryTree(目录树)合成**单个标签页面板**(位置 / 目录树 切换 + `+` 加位置),省 ~260px 给工作区;> 1400px 恢复并排。两组件各加 `embedded` 模式(去标题栏、宽度 100%)。
