...
- `ipc.ts` god-registrar:1376 行、87 个 `ipcMain.handle` 全部内联在一个 `registerIpcHandlers()`,另混 ~600 行业务逻辑(递归扫描 / 编码探测 / zip / reveal / wasm 资源缓存);`main.ts` 尾部混入 ~220 行 whale-audio 转码协议逻辑。待拆:按域分 `main/ipc/*.ts`,协议 handler 独立成模块。
...
- `reducers/settings.ts` god-slice:1203 行、53 字段混主题 / 列宽 / 外部二进制 / AI / keybinding / 地图;全扁平 setter 暂无正确性风险,可按域拆 3-4 个 slice。
...
- `FileList.tsx` 的 `useNow()` 每分钟 tick 在 `visible` memo deps 里:激活 `date:` / `smart:` 过滤时每分钟全可见行重渲(无过滤时返回原引用,影响受控)。
- `FileToolbar.tsx` 用 combined hook 订阅全量 context 只用 `refresh` / `loading`,可改订阅单片。
...
- ⏳ 新遗留:[smart-tags.ts:36](../src/shared/smart-tags.ts) 反向 import `../renderer/domain/calendar`(re-export `isPeriodTag`)—— 全库唯一 shared → renderer/domain 反向依赖,main 经 smart-tags 传递依赖渲染层模块,分层方向倒挂;把 `isPeriodTag` 挪回 shared 侧即可解开。
